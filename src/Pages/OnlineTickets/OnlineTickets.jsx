...
const OnlineTickets = () => {
  const [onlineTicketCount, setOnlineTicketCount] = useState(0);
  ...
  useEffect(() => {
    fetch("https://e-ticket-server-pi.vercel.app/totalOnlineTickets")
      .then((res) => res.json())
      .then((data) => setOnlineTicketCount(data));

    fetch("https://e-ticket-server-pi.vercel.app/tickets")
      .then((res) => res.json())
      .then((data) => {
        const onlineData = data.filter(
          (ticket) => ticket.ticketType === "online"
        );
        setTicket(onlineData);
        setFilteredTickets(onlineData);
      });
  }, []);
  ...
  const handlePrintPDF = () => {
    ...
    doc.setFontSize(18);
    doc.text("Online Ticket Report", 40, 40);
    doc.setFontSize(12);
    doc.text(`Total Online Tickets: ${filteredTickets.length}`, 40, 60);
    ...
    doc.save("Online_Ticket_Report.pdf");
  };
