...
const CounterNewTickets = () => {
  const [counterTicketCount, setCounterTicketCount] = useState(0);
  ...
  useEffect(() => {
    fetch("https://e-ticket-server-pi.vercel.app/totalCounter/unUsed")
      .then((res) => res.json())
      .then((data) => setCounterTicketCount(data));

    fetch("https://e-ticket-server-pi.vercel.app/tickets")
      .then((res) => res.json())
      .then((data) => {
        const filtered = data.filter(
          (t) => t.ticketType === "counter" && t.status === "unused"
        );
        setTickets(filtered);
  ...
  const handlePrintPDF = () => {
    ...
    doc.setFontSize(18);
    doc.text("New Counter Ticket Report", 40, 40);

    doc.setFontSize(12);
    doc.text(`Total New Counter Tickets: ${filteredTickets.length}`, 40, 60);
    ...
    doc.save("New_Counter_Ticket_Report.pdf");
  };
  ...
  return (
    <div>
      ...
      <h2 className="my-10 text-3xl text-center font-semibold">
        Total New Counter Tickets {counterTicketCount}
      </h2>
...
export default CounterNewTickets;
