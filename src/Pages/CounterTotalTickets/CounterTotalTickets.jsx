...
const CounterTotalTickets = () => {
  ...
  useEffect(() => {
    ...
    fetch("https://e-ticket-server-pi.vercel.app/tickets")
      .then((res) => res.json())
      .then((data) => {
        const filtered = data.filter((t) => t.ticketType === "counter");
        setTickets(filtered);
  ...
  const handlePrintPDF = () => {
    ...
    doc.setFontSize(12);
    doc.text(`Total Counter Tickets: ${filteredTickets.length}`, 40, 60);
  ...
  return (
    <div>
      ...
      <h2 className="my-10 text-3xl text-center font-semibold">
        Total Counter Tickets {counterTicketCount}
      </h2>
...
export default CounterTotalTickets;
